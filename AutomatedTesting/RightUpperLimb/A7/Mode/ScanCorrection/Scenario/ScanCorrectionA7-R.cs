...
public partial class FullScanCorrectionA7R
{
    private AltDriver altDriver;
    private ScanCorrectionHelpersA7R helpers; // Utiliser une instance des helpers pour accéder aux méthodes.

    [OneTimeSetUp]
    public void OneTimeSetUp()
    {
        altDriver = new AltDriver(host: ConnexionConfig.Host, port: ConnexionConfig.Port, appName: ConnexionConfig.AppName);
        helpers = new ScanCorrectionHelpersA7R(altDriver); // Initialisation avec le driver.
    }
    ...
    [Test]
    public void FullScanCorrectionA7Workflow()
    {
        ExecuteCleanupLevel();
        ExecutePoseLevel();
        ExecuteThumbLevel();
