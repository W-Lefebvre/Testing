...
public partial class FullScanCorrectionW3L
{
    ...
    private ScanCorrectionHelpersW3L helpers; // Utiliser une instance des helpers pour accéder aux méthodes.

    [OneTimeSetUp]
    public void OneTimeSetUp()
    {
        altDriver = new AltDriver(host: ConnexionConfig.Host, port: ConnexionConfig.Port, appName: ConnexionConfig.AppName);
        helpers = new ScanCorrectionHelpersW3L(altDriver); // Initialisation avec le driver.
    }
    ...
    [Test]
    public void FullScanCorrectionW3LWorkflow()
    {
        ExecuteCleanupLevel();
        ExecutePoseLevel();
        ExecuteSculptLevel();
