...
public partial class FullScanCorrectionH1L
{
    ...
    private ScanCorrectionHelpersH1L helpers; // Utiliser une instance des helpers pour accéder aux méthodes.

    [OneTimeSetUp]
    public void OneTimeSetUp()
    {
        altDriver = new AltDriver(host: ConnexionConfig.Host, port: ConnexionConfig.Port, appName: ConnexionConfig.AppName);
        helpers = new ScanCorrectionHelpersH1L(altDriver); // Initialisation avec le driver.
    }
    ...
    [Test]
    public void FullScanCorrectionH1LWorkflow()
    {
        ExecuteCleanupLevel();
        ExecutePoseLevel();
        ExecuteSculptLevel();
