...
public partial class FullScanCorrectionA5L
{
    ...
    private ScanCorrectionHelpersA5L helpers; // Utiliser une instance des helpers pour accéder aux méthodes.

    [OneTimeSetUp]
    public void OneTimeSetUp()
    {
        altDriver = new AltDriver(host: ConnexionConfig.Host, port: ConnexionConfig.Port, appName: ConnexionConfig.AppName);
        helpers = new ScanCorrectionHelpersA5L(altDriver); // Initialisation avec le driver.
    }
    ...
    [Test]
    public void FullScanCorrectionA5LWorkflow()
    {
        ExecuteCleanupLevel();
        ExecutePoseLevel();
        ExecuteSculptLevel();
    ...
    private void ExecuteCleanupLevel()
    {
        ...
        helpers.ClickSelectPartOfInterestArmLeftA1(); // Cannot use object here, use coordinate.
