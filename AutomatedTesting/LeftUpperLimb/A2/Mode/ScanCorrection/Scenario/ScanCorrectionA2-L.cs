...
public partial class FullScanCorrectionA2L
{
    ...
    private ScanCorrectionHelpersA2L helpers; // Utiliser une instance des helpers pour accéder aux méthodes.

    [OneTimeSetUp]
    public void OneTimeSetUp()
    {
        altDriver = new AltDriver(host: ConnexionConfig.Host, port: ConnexionConfig.Port, appName: ConnexionConfig.AppName);
        helpers = new ScanCorrectionHelpersA2L(altDriver); // Initialisation avec le driver.
    }
    ...
    [Test]
    public void FullScanCorrectionA2LWorkflow()
    {
        ExecuteCleanupLevel();
       // ExecutePoseLevel();
        ExecuteThumbLevel();
    ...
    private void ExecuteFullCleanupLevel()
    {
        ...
        helpers.ClickSelectPartOfInterestArmLeftA2();
        helpers.ClickQuitBtnObject();
