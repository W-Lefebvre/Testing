...
public partial class FullScanCorrectionA6L
{
    ...
    private ScanCorrectionHelpersA6L helpers; // Utiliser une instance des helpers pour accéder aux méthodes.

    [OneTimeSetUp]
    public void OneTimeSetUp()
    {
        altDriver = new AltDriver(host: ConnexionConfig.Host, port: ConnexionConfig.Port, appName: ConnexionConfig.AppName);
        helpers = new ScanCorrectionHelpersA6L(altDriver); // Initialisation avec le driver.
    }
    ...
    [Test]
    public void FullScanCorrectionA6LWorkflow()
    {
        ExecuteCleanupLevel();
        ExecutePoseLevel();
        ExecuteSculptLevel();
